...
const ChatBox = () => {
  ...
  const filteredConversations = Array.isArray(conversations)
    ? conversations.filter(
        (conv) =>
          conv.isClosed !== true && // đảm bảo chỉ loại bỏ những cuộc trò chuyện bị đóng
    ...
    <div className="flex h-[730px] bg-white font-sans">
      <div className="w-72 border-r bg-[#FAFAFA] p-4 overflow-y-auto">
        ...
        {filteredConversations.map((conv) => {
          const partnerName = conv.partnerName
  ? conv.partnerName + (conv.userType === 'partner' ? ' - Partner' : '')  // sửa ở đây
  : (conv.senderId === userId
      ? `Staff #${conv.recipientId}`
      : `User #${conv.senderId}` + (conv.userType === 'partner' ? ' - Partner' : ''));
  ...
  {conversation?.userType === 'partner' ? ' (partner)' : ''}   {/* sửa ở đây */}
</p>
  ...
  {msg.senderName}
  {msg.senderRole === 'staff' && ' - staff'}
  {msg.senderRole === 'partner' && ' - partner'}.  {/* sửa ở đây */}
</p>
