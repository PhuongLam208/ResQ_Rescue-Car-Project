...
const History = ({ customer }) => {
  ...
  const fetchHistory = async () => {
    try {
      const response = await managerApi.reqResQsAPI.findReqResQsByUser(customer.userId);
  ...
  const searchHistory = async () => {
    try {
      if (keyword.trim() == "") {
        ...
        const response = await managerApi.reqResQsAPI.searchCustomer(customer.userId, keyword);
        setHistory(response.data);
        setIsLoading(false);
      }
    } catch (err) {
  ...
  const handleViewDetail = async (his) => {
    setDetail(his);
    try {
      const response = await managerApi.reqResQsAPI.relatedRecordCheck(his.rrid);
    ...
    try {
      const response = await managerApi.extraSrvAPI.findExtrasByResResQ(his.rrid);
  ...
  const handleViewFeedback = async (his) => {
    try {
      const response = await managerApi.feedbackAPI.findFeedbacksByReqResQ(his.rrid);
